...
export default function CustomerChatPage() {
        ...
        newSocket.on('connect_error', async (error) => {
          ...
          if (error.message === 'Authentication error') {
            ...
            if (newSession?.access_token) {
              newSocket.auth = { token: newSession.access_token };
              newSocket.connect();
            }
          }
        });
  ...
  if (loading) {
    return <div className="dlb-loading">Loading your conversations...</div>;
  }
      ...
      {activeConversation ? (
        ...
        <div className="dlb-no-chat-selected">
          ...
          {conversations.length === 0 && (
            <p>You don't have any conversations yet. Place an order to start chatting with a seller.</p>
          )}
        </div>
      )}
    </div>
